...
int	main(int ac, char **av) {
	...
	} else if (ac == 3 && std::strlen(av[1]) && av[1][0])
		errorExit("input error: invalid option");
	else if (ac != 2)
	...
	if (input == "HELP") {
		std::cout << BLUE << std::endl << "usage:" << VIOLET << " ./computor [-v]" << GREEN <<" a * X^(n)" << CYAN <<" +" << GREEN <<" b * X^(n - 1) " <<CYAN <<"+" << GREEN << " ... " << CYAN <<"+" << GREEN << " n * X^0 " << YELLOW << "=" << GREEN <<" A * X^(N) " << CYAN <<"+" << GREEN <<" B * X^(N - 1)" << CYAN <<"+" << GREEN <<" ... " << CYAN << "+" << GREEN <<" N * X^0" << std::endl << std::endl;
